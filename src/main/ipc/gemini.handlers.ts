...
export function setupGeminiHandlers(ipcMain: IpcMain): void {
  ...
  ipcMain.handle(IPC_CHANNELS.AI_GENERATE_BATCH, async (_, words: string[], examplesCount: number) => {
    try {
      // Re-initialize in case API key was just set
      await initializeAI();
      return await geminiService.generateWordMeaningsBatch(words, examplesCount);
    } catch (error: any) {
